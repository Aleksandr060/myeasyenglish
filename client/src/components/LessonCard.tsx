...
export function LessonCard({ lesson }: { lesson: LessonSummary }) {
  ...
  return (
    <article
      className="flex h-full flex-col rounded-[28px] border p-5 shadow-card transition hover:-translate-y-1"
      data-surface="base"
    >
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-extrabold uppercase tracking-[0.24em] text-brand/70">
            {lesson.level} • {getQuestionTypeLabel(lesson.type, language)}
          </p>
